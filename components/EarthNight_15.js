...
const EarthNight_15 = () => {
  ...
  useEffect(() => {
    ...
    const sphere = new THREE.Mesh(
      new THREE.SphereGeometry(9, 250, 250),
      new ShaderMaterial({
    ...
    const animate = () => {
      ...
      camera.position.x =
        10 * Math.sin(Date.now() * 0.001) * Math.cos(Date.now() * 0.002) + 30;
      camera.position.y =
        20 * Math.sin(Date.now() * 0.003) * Math.cos(Date.now() * 0.004) + 50;
      camera.position.z =
        5 * Math.cos(Date.now() * 0.001) * Math.sin(Date.now() * 0.002) + 70;
